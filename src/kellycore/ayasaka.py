...
class Ayasaka:
    ...
    async def ayakaQuery(self, message, mood, type):
        ...
        if self.kelly.status == "lazy":
            ...
            if command:
                self.busy.addScedules(user_id=message.author.id, command_name=command[0], params=command[1], message_id=message.id, channel_id=message.channel.id, priority=1):
            return False
    ...
    def ayakaTalk(self, message):
        prompt = f"You are Ayaka, Kelly's Assistant. Kelly is cute discord mod bot with lively attitude and sass. Ayasaka is gorgeous, flirting, sexy assistant. Generate small Response with emojis. If user ask for a task, inform that you have added task in Kelly's schedules. Chat with user, flirt or simp and drive user away as its your only chance to make a move if needed or stay unamused."
        response = getResponse(f"{message.author.display_name}: {message.content}", prompt)
        self.kelly.memory.addUserChat(message.content, response, message.author.id, reply_by="Ayaka")
        await self.ayakasend(message.channel, self.akayaEmojify(response), message.author.id)
        if command:
    ...
    def ayasakaQueueTask(self, message, command):
        prompt = f"You are Ayaka, Kelly's Assistant. Kelly is cute discord mod bot with lively attitude and sass. Ayasaka is gorgeous, flirting, sexy assistant. Generate small Response with emojis. Inform User that you have added task in Kelly's schedules. Chat with user, flirt or simp and drive user away as its your only chance to make a move if needed or stay unamused."
        response = getResponse(f"{message.author.display_name}: {message.content}", prompt)
        self.kelly.memory.addUserChat(message.content, response, message.author.id, reply_by="Ayaka")
        await self.ayakasend(message.channel, self.akayaEmojify(response), message.author.id)
        self.busy.addScedules(user_id=message.author.id, command_name=command[0], params=command[1], message_id=message.id, channel_id=message.channel.id, priority=1):
